...
class appSpider(CrawlSpider):
    ...
    def parse_letter(self,response):
        selector = Selector(response)
        for href in selector.xpath('//a[starts-with(@title,"Browse More")]/@href').extract():
    ...
    def parse_page(self,response):
        ...
        nextLink = selector.xpath('//*[@class="paginate-more"]/@href').extract()
        if nextLink:
            nextLink = nextLink[0]
            yield Request(nextLink, callback=self.parse_page)
    ...
    def parse_dir_contents(self, response):
        ...
        if (response.xpath('//*[@id="left-stack"]/div[1]/ul/li[6]/text()').extract()):
        ...
        if (response.xpath('//*[@id="left-stack"]/div[2]/div[2]/@aria-label').extract()):
